from shared_functions import perform_similarity_search
...
def enhanced_rag_food_chatbot(collection):
    ...
    print("   Powered by Google's Gemini Model")
    print("="*70)
...
def prepare_context_for_llm(query:str,search_results:List[Dict]) -> str:
    if not search_results:
        return "No relevent food items found in the database"

    context_parts = []
    context_parts.append("Based on your query, here are the most relevent food options from our database")
...
def generate_fallback_response(query:str, search_results: List[Dict]) -> str:
    if not search_results:
        return "I couldnt find any food items matching your requirements, Please try again"
    ...
    if len(search_results) > 1:
        ...
        response_parts.append(f"Another good option is {second_choice['food_name']}, another {second_choice['cuisine_type']}")
...
def handle_enhanced_comparison_mode(collection):
    ...
    query1 = input("Enter the first food query: ").strip()
    query2 = input("Enter the second food query: ").strip()
